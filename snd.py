...
def playmusic(musicname, volume=1.0):
    ...
    SwitchingSongs = 1
    if music.get_busy():
        music.set_endevent(USEREVENT+3)
        music.fadeout(1000)
    else:
        prefvolume = [0, 0.6, 1.0][game.music]
        from game import base_dir, sep
        fullname = os.path.join(base_dir+sep+'data', 'music', musicname)
        music.load(fullname)
        music.play(-1)
...
def finish_playmusic():
    ...
    prefvolume = [0, 0.6, 1.0][game.music]
    from game import base_dir, sep
    fullname = os.path.join(base_dir+sep+'data', 'music', CurrentSong)
    music.load(fullname)
    music.play(-1)
    music.set_volume(prefvolume*CurrentVolume)
